...
const getConsistentImageSrc = () => {
  ...
  if (url.includes('picsum.photos')) {
    ...
    if (url.includes('?random=') || url.includes('&random=')) {
      ...
      const seedUrl = `https://picsum.photos/seed/${props.seed}/${dimensions.join('/')}`;
    ...
    if (url.match(/picsum\.photos\/\d+\/?\d*$/)) {
      ...
      const dimensions = pathParts.slice(-2); // Get width and height
      return `https://picsum.photos/seed/${props.seed}/${dimensions.join('/')}`;
    }
...
<template>
  <nuxt-img
    v-bind="{ ...props, src: finalSrc }"
    :alt="props.alt ?? config.public.appName"
    :loading="props.loading"
    decoding="async"
    :preload="props.loading === 'eager'"
    :fit="props.fit"
  />
</template>
